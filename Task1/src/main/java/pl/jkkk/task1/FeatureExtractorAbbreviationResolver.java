...
public class FeatureExtractorAbbreviationResolver {
    ...
    public FeatureExtractorAbbreviationResolver(final Set<String> keywordsTfIdf, final Set<String> keywordsTfcItfoc){
    ...
    public FeatureExtractor resolveAbbreviation(final String abbr) {
        ...
        switch(Integer.parseInt(params[0])){
            case 1: return new DocumentLengthFE();
            case 2: return new MostFrequentWordInDocumentFragmentFE(
                    Integer.parseInt(params[1].split("-")[0]),
                    Integer.parseInt(params[1].split("-")[1])
                );
            case 3: return new NumberOfKeywordsInDocumentFragmentFE(
                    params[2].equals("tfidf") ? keywordsTfIdf : keywordsTfcItfoc,
                    Integer.parseInt(params[1].split("-")[0]),
                    Integer.parseInt(params[1].split("-")[1])
                );
            case 4: return new RelativeNumberOfKeywordsInDocumentFragmentFE(
                    params[2].equals("tfidf") ? keywordsTfIdf : keywordsTfcItfoc,
                    Integer.parseInt(params[1].split("-")[0]),
                    Integer.parseInt(params[1].split("-")[1])
                );
            case 5: return new UniqueNumberOfKeywordsInDocumentFragmentFE(
                    params[2].equals("tfidf") ? keywordsTfIdf : keywordsTfcItfoc,
                    Integer.parseInt(params[1].split("-")[0]),
                    Integer.parseInt(params[1].split("-")[1])
                );
            case 6: return new MostFrequentKeywordInDocumentFragmentFE(
                    params[2].equals("tfidf") ? keywordsTfIdf : keywordsTfcItfoc,
                    Integer.parseInt(params[1].split("-")[0]),
                    Integer.parseInt(params[1].split("-")[1])
                );
